        ...
        with st.spinner("🕵️‍♂️ Analyzing visual & audio vectors..."):
            ...
            try:
                ...
                if video_file.state.name == "FAILED":
                    ...
                    # E. MULTI-MODEL FALLBACK ENGINE (Updated List)
                    # We use "Aliases" instead of specific dates to avoid 404s
                    models_to_try = [
                        "gemini-1.5-flash",    # The Global Standard (Fastest, Most Stable)
                        "gemini-2.0-flash",    # The New Flagship (If available)
                        "gemini-1.5-pro"       # The "Smart" Fallback (Higher limits, slower)
                    ]
                    ...
                    for model_name in models_to_try:
                        try:
                            # st.toast(f"Attempting with model: {model_name}...") # Debug
                            model = genai.GenerativeModel(model_name=model_name)
                            response = model.generate_content([prompt, video_file])
                            success_model = model_name
                            break # Success! Exit the loop
                        except Exception as e:
                            # Catch 404, 429, and 503 errors and just move to next model
                            last_error = e
                            time.sleep(1)
                            continue
